# Simulation Analysis for Treatment Selection
# This script simulates a two-stage design process for selecting one of three
# treatment arms and evaluates key metrics.

# Initialize Parameters
mu_comb <- list(
  c(0.24888, 0.24888 * 2 / 3, 0.24888 / 3), #Linear
  c(0.24888, 0, 0),                         #LFC
  c(0.2488000, 0.2488 / 2, 0.2488 / 2),     #Step
  c(0.2488000, 0.2388753, 0.1760137),       #Logistic
  c(0.24888, 0.0796, 0.0),                  #Convex
  c(0,0,0)                                  #Null Hypothesis
)  # Different treatment configurations

varS <- 0.2488    # Variance of the response
K <- length(mu_comb[[1]])  # Number of treatment arms in the first scenario

# Input parameters for decision boundaries and sample size
inputSA <- c(2.61, 0.06, 2.49, 0.54, 0.546, 331.7)
u1 <- inputSA[1]  # Upper threshold for Stage 1
l1 <- inputSA[2]  # Lower threshold for Stage 1
u2 <- inputSA[3]  # Upper threshold for Stage 2
h <- inputSA[4]   # Decision boundary
p <- inputSA[5]   # Proportion of samples in Stage 1
N <- inputSA[6]   # Total sample size
